...
class PolygonizerHead(MaskFormerHead):
    ...
    def __init__(self,
        ...
        self.dp_polygonize_head = DPPolygonizeHead(
            ...
            loss_poly_reg, loss_poly_cls, loss_poly_ang,
        )
    ...
    def _loss_by_feat_single(self, batch_gt_instances: List[InstanceData],
        ...
        if mask_targets.shape[0] == 0:
            ...
            # Find any trainable parameter in the model
            param = next((p for p in self.parameters() if p.requires_grad), None)
            if param is not None:
                dummy = param.sum() * 0.0
            else:
                # fallback: create a zero tensor with requires_grad=True (rarely needed)
                dummy = torch.zeros(1, device=mask_preds.device, requires_grad=True)
        ...
        losses_poly = {}  # Always define losses_poly
